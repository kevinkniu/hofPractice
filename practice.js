...
var sumTotal = function(products) {
  return _.reduce(products, function(sum, item) {
    return sum + Number(item.price.slice(1));
  }, 0);
};
...
var dessertCategories = function(desserts) {
  ...
  _.each(desserts, function(item) {
    catArray.push(item.type);
  });
  console.log(catArray);
  return _.reduce(catArray, function(object, type) {
...
var glutenFree = function(desserts) {
  return _.map(desserts, function(dessert) {
    ...
    if (dessert.ingredients.some(function(ingredient) {
...
var applyCoupon = function(groceries, coupon) {
  return _.map(groceries, function(item) {
    ...
    //originally was using this notation but eslint doesn't like `;
    item.salePrice = '$' + (Math.round(price * (1 - coupon) * 100) / 100).toFixed(2);
    return item;
  });
};
